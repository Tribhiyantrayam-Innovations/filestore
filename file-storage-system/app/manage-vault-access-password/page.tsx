...
export default function ManageVaultAccessPasswordPage() {
  ...
  const handleCreatePassword = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/create-vault-access-password", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        setSuccess("Vault access password created successfully!")
        setNewPassword("")
        setConfirmPassword("")
        setHasPassword(true)
        setTimeout(() => {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/update-vault-access-password", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        setSuccess("Vault access password updated successfully!")
        setCurrentPassword("")
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="text-center space-y-4">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full border border-white/20">
              ...
              <span className="text-white/90 text-sm font-medium">Vault Access Management</span>
            </div>
            <h1 className="text-4xl font-bold bg-gradient-to-r from-white via-purple-200 to-pink-200 bg-clip-text text-transparent">
              Manage Vault Access Password
            </h1>
            <p className="text-white/70 text-lg">Create or update your vault access password</p>
          </div>
                  ...
                  <Card className="bg-white/5 border-white/10">
                    <CardHeader>
                      <CardTitle className="text-white flex items-center gap-2">
                        ...
                        Create New Vault Access Password
                      </CardTitle>
                      <CardDescription className="text-white/70">
                        Set up a new password to secure your vault access
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                  ...
                  <Card className="bg-white/5 border-white/10">
                    <CardHeader>
                      <CardTitle className="text-white flex items-center gap-2">
                        ...
                        Update Vault Access Password
                      </CardTitle>
                      <CardDescription className="text-white/70">
                        Change your existing vault access password
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
